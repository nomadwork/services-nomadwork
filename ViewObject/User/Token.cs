...
namespace Nomadwork.ViewObject.User
{
    ...
    public class Token
    {
        public Token(bool authenticated, int expirationInSeconds, string accessToken)
        {
            ...
            Created = DateTime.Now.ToBr().ToString("yyyy-MM-dd HH:mm:ss");
            Expiration = (DateTime.Now.ToBr() + TimeSpan.FromSeconds(expirationInSeconds)).ToString("yyyy-MM-dd HH:mm:ss");
